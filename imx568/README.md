# IMX568 kernel driver

Installing and running the CircuitValley IMX568 driver on a Raspberry Pi.
The sensor is 2472 x 2064, 2 or 4 CSI-2 data lanes.
...
cd chc5_sensor_v4l2_drivers/imx568
./setup.sh
```
...
dtoverlay=imx568,cam0,always-on
```
...
```
dtoverlay=imx568,cam0,always-on,link-frequency=445500000
```
...
CircuitValley IMX568 modules fit a **37.125 MHz** oscillator, which is the
...
```bash
dmesg | grep -i imx568
media-ctl -p | grep -i imx568          # the sensor should appear as an entity
...
Streams on a Raspberry Pi 5 through both V4L2 and libcamera, in 10 and 12-bit,
with autogain, manual exposure and a three-lamp colour calibration completed.
The module tested has an IR-cut filter fitted.
