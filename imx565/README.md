# IMX565 kernel driver

Installing and running the CircuitValley IMX565 driver on a Raspberry Pi.
The sensor is 4128 x 3008, 2 or 4 CSI-2 data lanes.
...
cd chc5_sensor_v4l2_drivers/imx565
./setup.sh
```
...
dtoverlay=imx565,cam0,always-on
```
...
```
dtoverlay=imx565,cam0,always-on,link-frequency=445500000
```
...
CircuitValley IMX565 modules fit a **37.125 MHz** oscillator, which is the
...
```bash
dmesg | grep -i imx565
media-ctl -p | grep -i imx565          # the sensor should appear as an entity
...
Streams on a Raspberry Pi 5 through both V4L2 and libcamera, in 8, 10 and
12-bit, with autogain, manual exposure and a three-lamp colour calibration
completed. The module tested has an IR-cut filter fitted.

- This is a Pregius S global-shutter part. Geometry comes from the hardware ROI
  engine, so the driver advertises a continuous size range rather than a list of
  discrete modes.
- Master mode free-run only. Trigger and slave modes are not implemented.
