# IMX477 kernel driver

Installing and running the CircuitValley IMX477 driver on a Raspberry Pi.
The sensor is 4056 x 3040, 2 or 4 CSI-2 data lanes.
...
cd chc5_sensor_v4l2_drivers/imx477
./setup.sh
```
...
```
dtoverlay=imx477,cam0,always-on,link-frequency=450000000
```
...
CircuitValley IMX477 modules fit a **24 MHz** oscillator, which is the
...
```bash
dmesg | grep -i imx477
media-ctl -p | grep -i imx477          # the sensor should appear as an entity
...
Tuning exists upstream for both RP1 and unicam. Our fork patches only the
CamHelper's gain functions, leaving the upstream tuning files as they are.

## Status and known gaps

Streams on a Raspberry Pi 5 through both V4L2 and libcamera, on a 4-lane board,
with autogain, manual exposure and a three-lamp colour calibration completed.

- **Raspberry Pi OS already ships an in-kernel imx477 driver.** `setup.sh`
  forces the DKMS install over it; check with
  `modinfo imx477 | grep filename` that the module resolves under
  `updates/dkms/` and not the stock path.
- **This driver and the libcamera fork also take over a genuine Raspberry Pi HQ
  Camera** on the same system, since both are keyed on the same sensor name. The
  patched CamHelper changes the gain law for it too. Do not install this sensor
  on a system where an HQ Camera must behave exactly as Raspberry Pi ships it.
- The receiver's low-level FIFO panic counter ticks during streaming with no
  frame loss observed.
