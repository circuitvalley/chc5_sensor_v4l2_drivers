# IMX294 kernel driver

Installing and running the CircuitValley IMX294 driver on a Raspberry Pi.
The sensor is 4176 x 2824, 4-lane only.
...
cd chc5_sensor_v4l2_drivers/imx294
./setup.sh
```
...
dtoverlay=imx294,cam0,always-on
```
...
```
dtoverlay=imx294,cam0,always-on,link-frequency=480000000
```
...
CircuitValley IMX294 modules fit a **24 MHz** oscillator, which is the
...
driver supports 24 MHz and 72 MHz.
...
```bash
dmesg | grep -i imx294
media-ctl -p | grep -i imx294          # the sensor should appear as an entity
...
- **The default 480 MHz link frequency does not work on a Pi 5.** The sensor
  streams but the receiver counts no packets and no errors. The cause is the RP1
  D-PHY frequency band selected for 960 Mbps, not the sensor. Programming the
  receiver for 1200 Mbps works, and sensor rates from 864 to 1728 Mbps/lane all
  stream cleanly at that setting. Add `link-frequency=600000000` to the overlay.
- A plain `S_FMT` at 1920x1080 gives a centre crop, not a binned full field of
  view. The binned full-field modes are not advertised in a way libcamera picks.
- Horizontal and vertical flip controls are not implemented.
