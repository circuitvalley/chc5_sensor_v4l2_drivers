# IMX585 kernel driver

Installing and running the CircuitValley IMX585 driver on a Raspberry Pi.
...
cd chc5_sensor_v4l2_drivers/imx585
./setup.sh
```
...
```
dtoverlay=imx585,cam0,always-on,link-frequency=445500000
```
...
CircuitValley IMX585 modules fit a **37.125 MHz** oscillator, which is the
...
driver supports 24 MHz, 37.125 MHz and 72 MHz.
...
The monochrome variant streams as Y12 and appears to libcamera as `MONO`.
At 891 Mbps/lane the receiver reported zero discards.
...
- `rotation` and `orientation` are carried in the device tree but the driver
  does not yet publish the matching V4L2 controls.
